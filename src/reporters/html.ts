...
import { HttpServer, MultiMap, assert, calculateSha1, getPackageManagerExecCommand, copyFileAndMakeWritable, gracefullyProcessExitDoNotHang, removeFolders, sanitizeForFilePath, toPosixPath } from 'patchright-core/lib/utils';
import { colors } from 'patchright-core/lib/utils';
import { open } from 'patchright-core/lib/utilsBundle';
import { mime } from 'patchright-core/lib/utilsBundle';
import { yazl } from 'patchright-core/lib/zipBundle';
...
import type { ZipFile } from 'patchright-core/lib/zipBundle';
...
class HtmlBuilder {
  ...
  async build(metadata: Metadata, projectSuites: api.Suite[], result: api.FullResult, topLevelErrors: api.TestError[], machines: MachineEndResult[]): Promise<{ ok: boolean, singleTestId: string | undefined }> {
    ...
    // Copy app.
    const appFolder = path.join(require.resolve('patchright-core'), '..', 'lib', 'vite', 'htmlReport');
    await copyFileAndMakeWritable(path.join(appFolder, 'index.html'), path.join(this._reportFolder, 'index.html'));

    // Copy trace viewer.
    if (this._hasTraces) {
      const traceViewerFolder = path.join(require.resolve('patchright-core'), '..', 'lib', 'vite', 'traceViewer');
